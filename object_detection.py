...
class ObjectDetection:
    def __init__(self, weights_path="dnn_model/yolov4.weights", cfg_path="dnn_model/yolov4.cfg"):
        ...
        # Prefer GPU (CUDA) if available; otherwise use CPU explicitly.
        # The backend/target calls may not throw immediately when CUDA is missing,
        # so proactively decide based on CUDA device count.
        use_cuda = False
        try:
            # On builds without CUDA this typically returns 0 or raises.
        ...
        if use_cuda:
            ...
            print("CUDA not available; using CPU backend for OpenCV DNN")
